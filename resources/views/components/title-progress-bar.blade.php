...
<div class="flex flex-wrap items-end justify-between gap-1">
    <div class="flex gap-5 max-lg:flex-col lg:-translate-x-10">
        <div class="flex items-center gap-5 lg:items-center">
            @if ($isAnchor)
                <a href="{{ $attributes->get('href', ':href', 'v-bind:href') }}" aria-label="{{ $attributes->get('aria-label') }}" class="max-lg:size-14 max-lg:flex max-lg:items-center max-lg:justify-center max-lg:rounded-full max-lg:border">
        ...
        <x-rapidez-ct::title {{ $attributes->except(['checkoutSteps', 'currentStep', 'currentStepKey', 'href', ':href', 'v-bind:href', 'aria-label']) }}>
            {{ $slot }}
        </x-rapidez-ct::title>
    </div>
