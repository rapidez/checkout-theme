<label {{ $attributes->only('class')->class('relative flex w-full cursor-pointer items-center justify-start gap-x-3 rounded border bg-white px-5 sm:px-7 py-7 text-ct-sm text-ct-primary') }}>
    <span class="flex aspect-square size-6 items-center justify-center rounded-full border bg-ct-white">
        <input type="radio" {{ $attributes->except('class') }} class="peer size-3 border-none text-ct-accent transition checked:bg-none focus:ring-0 focus:ring-offset-0" />
