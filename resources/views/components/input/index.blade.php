...
<label {{ $attributes->only(['v-if', 'v-else', 'v-else-if', 'class'])->merge(['class' => 'relative flex flex-col gap-y-2 text-sm']) }}>
    ...
    <input {{ $attributes->merge([
        ...
        'class' => 'rounded peer border border-border bg-white py-4 px-5 text-sm outline-none !ring-0 transition-all placeholder:text-ct-inactive focus:border-ct-primary disabled:bg-ct-disabled disabled:text-ct-inactive font-medium disabled:pr-12',
    ]) }}>
    {{ $slot }}
