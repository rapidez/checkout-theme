...
<label {{ $attributes->only(['v-if', 'v-else', 'v-else-if', 'class'])->merge(['class' => 'relative flex flex-col gap-y-2 text-sm']) }}>
    ...
    <input {{ $attributes->except(['v-if', 'v-else', 'v-else-if', 'class'])->merge([
        ...
        'class' => 'rounded border border-border bg-white py-4 px-5 text-sm outline-none !ring-0 transition-all placeholder:text-ct-inactive focus:border-ct-primary disabled:bg-ct-inactive-200 disabled:text-ct-inactive font-medium',
    ]) }}>
    {{ $slot }}
