...
<label v-bind:for="{{ $id }}" class="pointer-events-none fixed inset-0 z-50 cursor-pointer bg-ct-neutral/60 opacity-0 transition peer-checked:pointer-events-auto peer-checked:opacity-100"></label>
<div class="pointer-events-none fixed top-0 -right-full z-50 flex h-screen w-full max-w-md flex-col bg-white transition-[right] duration-300 peer-checked:pointer-events-auto peer-checked:right-0">
    <div class="flex w-full items-center justify-center bg-ct-neutral py-4 px-5 text-lg font-bold text-white">
        <div>{{ $title }}</div>
